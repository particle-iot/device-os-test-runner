...
export class Runner {
	...
	async _build() {
		...
		for (let i = 0; i < apps.length; ++i) {
			...
			for (let platform of apps[i].platforms) {
	...
	_applySuiteConfig() {
		...
		for (let suite of rootSuite.suites) {
			...
			if (config.fixtures && config.fixtures.length) {
				...
				const suiteDir = path.dirname(particle.file);
				let suiteApps = this._builder.findApps(suiteDir);
				suiteApps = new Set(suiteApps.map(appDir => path.basename(appDir)));
				// Ignore application name if the suite requires only one fixture
				if (suiteApps.size !== 1 || particle.fixtures.length !== 1) {
					particle.fixtures.forEach(f => {
						if (!suiteApps.has(f.app)) {
							throw new SpecError(`No application found for the fixture "${f.name}"`, particle.file);
						}
					});
				}
			} else {
