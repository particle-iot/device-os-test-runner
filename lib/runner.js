...
export class Runner {
	constructor({ log }) {
		...
		this._appBuilder = null; // Application builder
	...
	async shutdown() {
		...
		this._mocha = null;
		if (this._appBuilder) {
			await this._appBuilder.shutdown();
			this._appBuilder = null;
		}
		if (this._devMgr) {
	...
	_initTestMatrix() {
		...
		for (let srcSuite of rootSuite.suites) {
			...
			for (let platform of particle.platforms) {
				...
				this._initPlatformSuite(platformSuite);
