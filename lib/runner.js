	...
	async _build() {
		...
		for (let i = 0; i < apps.length; ++i) {
			...
			for (const platform of apps[i].platforms) {
				...
				// Copy the application ELF built alongside the binary: it carries the DWARF
				// info and the module hash of the flashed binary, and is registered for panic
				// info symbolication when this prebuilt binary is used by a subsequent run
				const srcElf = srcBin.replace(/\.bin$/, '.elf');
				if (fs.existsSync(srcElf)) {
					fs.copyFileSync(srcElf, path.join(destDir, path.basename(srcElf)));
				}
			}
			if (i !== apps.length - 1) {
				console.log();
