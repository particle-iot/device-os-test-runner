...
class Runner {
	...
	_hasSuiteRetriesConfigured() {
		return this.suites.some(suite => !suite.particle.exclude && suite.particle.suiteRetries > 0);
	}
	...
	_selectSuiteForRun({ file, title }) {
		const rootSuite = this._mocha.suite;
		rootSuite.suites = rootSuite.suites.filter(suite => {
			return suite.particle && suite.particle.file === file && suite.title === title;
		});
	}
	...
	async _runWithSuiteRetries() {
		...
		for (const suite of plan) {
			...
			for (let attempt = 0; attempt <= suite.retries; ++attempt) {
				try {
					await this.init();
					this._selectSuiteForRun(suite);
					suiteOk = await this._run();
					lastError = null;
				} catch (e) {
					lastError = e;
					suiteOk = false;
				...
				if (attempt < suite.retries) {
					this._log.warn(`Retrying suite: ${suite.title} (${attempt + 1}/${suite.retries})`);
				}
			}
			if (!suiteOk) {
				ok = false;
				if (lastError && isInternalError(lastError)) {
					this._log.warn(`Suite failed after retries: ${suite.title}`);
				}
			}
		}
		return ok;
