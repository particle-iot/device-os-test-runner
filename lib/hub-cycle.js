...
// Cycle the USB hub port with uhubctl. Returns true if the cycle actually
// happened, false if the feature is disabled or uhubctl isn't available so
// callers can log the original error instead of pretending recovery happened.
async function cycleUSBwithPortPath(portPath, { delaySec = 2 } = {}) {
	if (!config.get('uhubctl')) {
		return false;
	}
