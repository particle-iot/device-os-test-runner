...
function parsePlatform(arg) {
	...
	if (not) {
		return PLATFORMS.filter(p => !p.has(arg));
	}
	return platformsForTag(arg);
}

/**
 * Parse arguments of the platform() function.
 *
...
export function parsePlatforms(args) {
	const ps = new Map();
	args.forEach(arg => {
		const ps2 = parsePlatform(arg);
		ps2.forEach(p => ps.set(p.id, p));
	});
