 ...
 *
 * @param {String} extTag Platform tag.
 * @returns {Array<Platform>}
 */
export function platformsForTag(extTag) {
	if (extTag.startsWith('!')) {
		extTag = arg.substring(1);
		return PLATFORMS.filter(p => !p.has(extTag));
	}
	const ps = PLATFORMS_BY_TAG.get(extTag);
	if (!ps) {
		throw new Error(`Unknown platform tag: ${extTag}`);
	}
	return ps;
}
...
export function platformTags() {
	return Array.from(PLATFORMS_BY_TAG.keys());
}
