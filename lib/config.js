...
const DEFAULT_TEST_DIR = 'test/integration'; // Relative to the Device OS directory
const DEFAULT_BUILD_DIR = `build/target/${DEFAULT_TEST_DIR}`; // Relative to the Device OS directory
...
function updateConfig(args) {
	...
	if (buildDir) {
		...
		if (!buildDir) {
			if (deviceOsDir) {
				buildDir = path.join(deviceOsDir, DEFAULT_BUILD_DIR);
			} else {
