...
const config = convict({
	...
	elfDirs: {
		doc: 'Directories with firmware ELF files used to symbolicate panic info',
		format: 'string-array',
		default: []
	},
	targetDir: {
	...
	suiteRetries: {
		doc: 'Default number of retries for a failed test suite',
		format: 'nat',
		default: 0
	},
	network: {
	...
	uhubctl: {
		doc: 'Enable use of uhubctl to power cycle devices',
		format: Boolean,
		default: false
	}
});
...
run [--test-dir=DIR] [--device-os-dir=DIR] [--binary-dir=DIR] [--report-file=FILE] [--dry] [--suite-retries=N] [filters...]
    Run tests.
...
--suite-retries=N
    Retry a failed suite up to N additional times.
...
function parseCmdLine() {
	return parseArgs(process.argv.slice(2), {
		string: ['_', 'grep', 'config-file', 'test-dir', 'binary-dir', 'elf-dir', 'device-os-dir', 'target-dir', 'report-file', 'network', 'suite-retries'],
		boolean: ['build', 'dry', 'fixtures', 'tags', 'combine', 'version', 'help', 'json', 'verbose', 'flash', 'uhubctl'],
		default: {
			'flash': true
		},
		alias: {
...
function updateConfig(args) {
	...
	const elfDirs = args['elf-dir'] || [];
	const elfDirList = Array.isArray(elfDirs) ? elfDirs : [elfDirs];
	config.set('elfDirs', elfDirList.filter((d) => d).map((d) => path.resolve(d)));
	// Target report file
	...
	if (args['suite-retries'] !== undefined) {
		config.set('suiteRetries', Number(args['suite-retries']));
	}
...
function loadConfigFiles(args) {
	...
	if (!files) {
		...
		files = glob.sync('*' + CONFIG_FILE_SUFFIX, {
			cwd: dir,
			nodir: true,
			absolute: true
		});
