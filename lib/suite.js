...
class PlatformSuite {
	...
	async _initDevices() {
		...
		if (fixtures.length) {
			...
			this._ctx.devices.push(dev);
			this._deviceLog(dev).verbose(`Target device: ${dev.displayName}`);
			dev.on('_attached', (arg) => { this._log.verbose(`_attach ${arg}`)});
			dev.on('_detached', (arg) => { this._log.verbose(`_detach ${arg}`)});
			dev.on('_opening', (arg) => { this._log.verbose(`_opening ${arg}`)});
			dev.on('_closing', (arg) => { this._log.verbose(`_closing ${arg}`)});
		}
	}
