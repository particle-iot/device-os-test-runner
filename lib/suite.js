'use strict';
const { TestResult, formatPanicInfo } = require('./device');
const { PanicResolver } = require('./panic_resolver');
...
class PlatformSuite {
	...
	async init(ctx) {
		...
		this._panicResolver = new PanicResolver({
			log: this._log,
			deviceOsDir: this._ctx.deviceOsDir,
			binaryDir: this._ctx.binaryDir,
			elfDirs: this._ctx.elfDirs
		});
		// Initialize devices under test
		await this._initDevices();
		if (!this._ctx.dryRun) {
			// Consume a panic record left over on the device before this suite started - e.g.
			// from the previous suite or a flashing step. The record is discarded: any panic
			// observed from now on is caused by this suite and attributed to the test that
			// observes it. This is done before flashing, as flashing itself can trigger a
			// panic on the device
			for (const dev of this._ctx.devices) {
				dev.setPanicResolver(this._panicResolver);
				const stalePanic = await dev.fetchAndClearPanicInfo().catch(e => {
					this._deviceLog(dev).debug(`Failed to check for a leftover panic record: ${e.message}`);
					return null;
				});
				if (stalePanic) {
					this._deviceLog(dev).warn(`Panic recorded before suite start: ${formatPanicInfo(stalePanic)}`);
				}
			}
	...
	async runTest(test) {
		...
		// A panic that happened since the previous test - during a JavaScript test body, a
		// device test, or between tests - only left the retained panic record behind; the
		// record is checked here, before the next test starts, so the panic is captured no
		// matter where it happened
		for (const dev of (devs && devs.length ? devs : this._ctx.devices) || []) {
			const err = await dev.checkPanicAfterTest().catch(e => {
				this._deviceLog(dev).debug(`Panic check failed: ${e.message}`);
				return null;
			});
			if (err) {
				throw err;
		...
		try {
			...
			const ps = devs.map(async (dev) => {
				...
				if (r.panicInfoText) {
					test.particle.data = { ...test.particle.data, panic: r.panicInfo || r.panicInfoText };
				}
	...
	async _flashAppForPlatform(platform) {
		...
		const ps = devApps.map(async ({ dev, app }) => {
			try {
				...
				// The sibling ELF carries the DWARF info for the app that was just flashed
				const elfPath = app.path.replace(/\.bin$/, '.elf');
				if (this._panicResolver && fs.existsSync(elfPath)) {
					this._panicResolver.addElf(elfPath);
				}
			} catch (err) {
