	...
	async runTest(test) {
		...
		try {
			...
			if (!test.parent.particle.suiteInitialized) {
				for (const dev of devs) {
					const stalePanic = await dev.fetchAndClearPanicInfo().catch(() => null);
					if (stalePanic) {
						this._deviceLog(dev).warn(`Panic recorded before suite start: ${stalePanic.text || stalePanic.code}`);
					}
