...
class DeviceManager {
	constructor({ apiClient, log }) {
		...
		this._log.trace = this._log.debug;
		particleUsb.config({
			log: {
				trace: (...args) => { this._log.verbose(...args) },
				info: (...args) => { this._log.verbose(...args) },
				warn: (...args) => { this._log.verbose(...args) },
				error: (...args) => { this._log.verbose(...args) }
			}
		});
	}
