...
const DEVICE_REOPEN_DELAY = 1000;
...
class Device extends EventEmitter {
	constructor({ id, name, platform, portPath, log }) {
		...
		this._lastOpen = 0; // Time when the device was last opened
		this._onAttachFn = (usbDev) => { // Listener for 'attach' events
			await this._onAttach(usbDev);
		};
		this._onDetachFn = (usbDev) => { // Listener for 'detach' events
			await this._onDetach(usbDev);
		};
	}

	async init() {
		const devs = await particleUsb.getDevices();
		this._attached = devs.some(dev => usbDevicePortPath(dev) === this._portPath);
	...
	async reset() {
		...
		try {
			...
			try {
				...
				// Forced reset always fails, so ignore the second error
				throw err;
			}
		} finally {
			await this._close();
	...
	async _open() {
		...
		while (!canOpen()) {
			const p = new Promise((resolve, reject) => {
				const onChange = () => {
					if (canOpen()) {
						clearTimeout(timer);
						this._off('_opening', onChange);
						this._off('_closing', onChange);
						resolve();
					}
				};
				const onTimeout = () => {
				...
				const timer = setTimeout(onTimeout, Math.min(0, timeoutAt - Date.now()));
			});
			await p;
		}
		this._opening = true;
		...
		try {
			...
			if (!this._usbDev || !this._usbDev.isOpen) {
				...
				for (;;) {
					try {
						const devs = particleUsb.getDevices();
