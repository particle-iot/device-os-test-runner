...
const ECHO_REQUEST_TYPE = 1; // ctrl_request_type::CTRL_REQUEST_ECHO
			...
			if (!this._usbDev || !this._usbDev.isOpen) {
				...
				for (;;) {
					...
					try {
						...
						await dev.open();
						try {
							// FIXME: The first request sent to the device right after it was opened may fail
							// with a "Request was cancelled" error. As a workaround, we send a dummy request
							// to the device and ignore the result
							await dev.sendControlRequest(ECHO_REQUEST_TYPE, 'hello', { timeout: 1000, pollingPolicy: 200 });
						} catch (err) {
							if (!(err instanceof particleUsb.TimeoutError || (err instanceof particleUsb.DeviceError &&
