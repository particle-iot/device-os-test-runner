import { parsePlatforms, platformForId, isKnownPlatformId } from './platform';
...
const DEFAULT_OPEN_TIMEOUT = 10000;
...
export class Device {
	...
	async flash(binFile) {
		...
		try {
			...
			usbDev = await this._openDevice(this._id);
	...
	async _openDevice() {
		const timeout = Date.now() + DEFAULT_OPEN_TIMEOUT;
		do {
			let usbDev = null;
			try {
...
export class Devices {
	...
	async init() {
		let platforms = parsePlatforms(config.get('platforms')); // Enabled platforms
		platforms = platforms.reduce((set, p) => set.add(p.id), new Set());
		let devs = await this._getUsbDevices();
		await this._initDeviceNames(devs);
		devs = this._initFixtures(devs, platforms);
		this._initDevicePool(devs, platforms);
	}
	...
	_initFixtures(devs, enabledPlatforms) {
