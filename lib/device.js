...
const DEVICE_OPEN_TIMEOUT = 10000;
...
export class Device {
	...
	async runTest(name, conf) {
		this._log.debug(`Running device test: ${name}`);
		const req = { c: 'i' }; // Init suite
		switch (conf.systemMode) {
		...
		do {
			...
			if (rep.result !== RequestResult.STATUS_RUNNING) {
				ok = (rep.result === RequestResult.STATUS_PASSED);
				break;
			}
			await delay(250);
