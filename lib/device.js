...
const DEVICE_FLASH_RETRIES = 3;
const DEVICE_FLASH_MIN_RETRY_DELAY = 1000;
const DEVICE_FLASH_MAX_RETRY_DELAY = 3000;
// Up to 2x our cellular registration timeout
const DEFAULT_TEST_TIMEOUT = 20 * 60 * 1000;
...
class Device extends EventEmitter {
	...
	async flash(binFile) {
		...
		let retries = DEVICE_FLASH_RETRIES;
		let lastError = null;
		while (retries-- > 0) {
			try {
				await this._open();
				...
				this._willDetach = false;
				lastError = err;
				this._log.debug(`Failed to flash device: ${err.message}`);
			}
			...
			try {
				...
				this._log.debug(`Failed to reset device: ${err.message}`);
			}

			await delay(DEVICE_FLASH_MIN_RETRY_DELAY + Math.ceil(Math.random() * (DEVICE_FLASH_MAX_RETRY_DELAY -
					DEVICE_FLASH_MIN_RETRY_DELAY)));
		}
		if (lastError) {
			throw lastError;
		}
	...
	async _request(req, { timeout = DEFAULT_REQUEST_TIMEOUT } = {}) {
		...
		let close = false;
		try {
			const rep = await this._usbDev.sendControlRequest(REQUEST_TYPE, JSON.stringify(req), { timeout });
			if (rep.data) {
