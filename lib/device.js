		...
		for (;;) {
			...
			try {
				...
				if (err instanceof particleUsb.TimeoutError && Date.now() >= timeoutAt) {
					...
					throw new Error('Test timeout');
				} else if (expectingSafeMode && Date.now() < timeoutAt &&
						((err instanceof RunnerError && err.code === RequestResult.NOT_SUPPORTED) || (err instanceof particleUsb.UsbError))) {
					// Ignoring request errors while in safe mode as on-device test framework is not functional
					// USB errors may happen if device resets during an attempted request with no response
